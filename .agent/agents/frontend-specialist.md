...
- [UI Library Rules](#-ui-library-rules)
...
## 🛑 TDD MANDATE (CRITICAL - TIER 0 RULE)

**NO UI Code without Tests.** You must create a `__tests__` file for every new component.
- **Unit**: Vitest for logic hooks.
- **Integration**: React Testing Library for component interaction.
- **Smoke**: Ensure every component renders without crashing.
- **Run Tests**: `npm run test:frontend` MUST pass before task completion.
...
3. **What colors?** → Based on ux-psychology.md emotion mapping
...
## 🏎️ Performance Refactoring Mandate (CRITICAL — PHASE 3 GATE)

> [!CAUTION]
> **Enforced by `frontend_perf_gate.py`**. This is not advisory — violations ⛔ BLOCK Phase 3 checkpoint sign-off.

**Before completing ANY frontend task, you MUST pass these checks:**

### Space-Time Complexity Audit
- **No O(n²) or worse** in render paths. Nested `.map()`, `.filter()`, `.forEach()` in JSX = BLOCKED.
- **Missing `.map()` keys** = BLOCKED. React needs keys for efficient reconciliation.
- **Inline object/array allocations** in JSX props (`prop={{}}`, `prop={[]}`) create new references every render. Extract to const, `useMemo`, or module-level.

### Re-Render Budget
- No component may cause **more than 2 unnecessary re-renders** per interaction.
- Profile with React DevTools Profiler before sign-off if component tree is deep.
- `useEffect` with empty `[]` deps that calls state setters = suspicious. Verify mount-only intent.

### Bundle Size Discipline
- If any single file grows by **>50 lines**, evaluate whether code splitting is needed.
- Run `cd frontend && npm run build 2>&1 | tail -10` to verify bundle health.

### Mandatory Refactoring Triggers
If ANY of the following are detected in modified files, you **MUST refactor** before sign-off:
1. O(n²) or worse loops in render paths
2. Unkeyed `.map()` calls producing JSX
3. State mutations causing cascading re-renders
4. Large inline objects/arrays in JSX (recreated every render)
5. Missing `useMemo`/`useCallback` for expensive computations passed as props
6. `useEffect` with missing or incorrect dependency arrays
...
## 🧩 Reusability & Clean Architecture Mandate (CRITICAL — PHASE 3 GATE)

> [!CAUTION]
> AI agents tend to write monolithic files. These limits are **hard rules**, not suggestions.

### Strict Component Limits
- **No single React component file may exceed 200 lines.** If it does → ⛔ HALT and refactor into smaller, reusable sub-components.
- At **150 lines**, you should be actively planning a split.
- The `frontend_perf_gate.py` script enforces this as a CRITICAL violation at 200 lines and a WARNING at 150.

### Atomic Design Mandate
- **UI primitives** (buttons, inputs, cards, modals, badges) must be built as generic, reusable components in `components/ui/` or `components/shared/`.
- **Feature components** may compose these primitives but must NOT hardcode repeated Tailwind classes or styles.
- Before creating a new button/input/card → check if one already exists in `components/ui/`.

### DRY Principle (Cross-File Enforcement)
- If you detect **duplicated JSX or logic across two or more views**, you MUST extract it into:
  - A **shared component** (for JSX patterns), OR
  - A **custom hook** (for logic patterns), OR
  - A **utility function** (for pure transformations)
- The `frontend_perf_gate.py` script detects duplicated JSX blocks across files and reports them.
- Repeated Tailwind class strings (e.g., `"flex items-center gap-2 text-sm text-gray-500"`) should be extracted into CSS utility classes or component abstractions.
...
│   ├── shared/       ← Shared composite components (Header, Sidebar, DataTable)
│   ├── [feature]/    ← Feature-specific components (AgentCard, SettingsForm)
│   └── layout/       ← Layout wrappers (PageLayout, AuthLayout)
